...
count_plankton %>% 
  ...
  mutate(Diff_year = Year - lag(Year),
         ...
         Rate_percent = (Diff_growth / Diff_year)/lag(total_count) * 100) -> growth_dat
...
clean_growth_dat %>%
  group_by(Year) %>%
  summarize(diversity_index = diversity(total_count, index = "shannon")) -> div_clean_dat
...
write.csv(div_clean_dat, file = 'div_clean_dat_1991to2018.csv')
...
# time series
```{r}
atmo_data_medians <- read.csv("atmo_data_medians_1991-2018.csv")

ts_plankton <- ts(data = div_clean_dat$diversity_index, 
                  start = c(1991))
ts_plankton

ts_atmo <- ts(data = atmo_data_medians$median_PCO2_TEQ,
              start = c(1991))
...
plot(ts_atmo, 
     ylab = "pCO2 (atm)")
plot(cbind(ts_plankton, ts_atmo), yax.flip = TRUE, 
     main = "Time Series of Plankton Diversity and pCO2",
     xlab = "Time"
     )


ccf(ts_atmo, ts_plankton, ylab = "Cross-correlation")
